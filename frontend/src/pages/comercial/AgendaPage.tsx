...
const TYPE_CONFIG: Record<EventType, { icon: ReactNode; bg: string; color: string }> = {
  reuniao: { icon: <Users size={12} />, bg: 'rgba(43,22,92,0.12)', color: '#8b5cf6' },
...
export default function AgendaPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div><p className="text-sm font-semibold mb-3" style={{ color: 'var(--erp-text)' }}>{fmtDay(selectedDay)}{selectedDay === todayIso() && <span className="ml-2 rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: 'var(--erp-violet-dim)', color: 'var(--erp-violet-light)' }}>Hoje</span>}</p>{loading ? <div className="py-12 text-sm" style={{ color: 'var(--erp-text-muted)' }}>Carregando agenda...</div> : todayEvents.length === 0 ? <div className="flex flex-col items-center justify-center gap-3 rounded-2xl py-14" style={{ border: '1px dashed var(--erp-border)' }}><Users size={28} style={{ color: 'var(--erp-text-dim)' }} /><p className="text-sm" style={{ color: 'var(--erp-text-muted)' }}>Nenhum lead com contato agendado neste dia</p></div> : <div className="space-y-2">{todayEvents.map((event) => { const tc = TYPE_CONFIG[event.type]; return <div key={event.id} className="flex items-center gap-4 rounded-2xl px-4 py-3" style={{ background: 'var(--erp-surface)', border: '1px solid var(--erp-border)' }}><div className="flex-shrink-0 w-12 text-center"><span className="text-sm font-bold tabular-nums" style={{ color: 'var(--erp-text)' }}>{event.time}</span></div><div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl" style={{ background: tc.bg, color: tc.color }}>{tc.icon}</div><div className="flex-1 min-w-0"><p className="text-sm font-medium" style={{ color: 'var(--erp-text)' }}>{event.title}</p><p className="text-xs" style={{ color: 'var(--erp-text-muted)' }}>{event.contact} - {event.company}</p></div><div className="flex-shrink-0"><span className="flex items-center gap-1 text-xs font-medium" style={{ color: event.confirmed ? '#34d399' : '#fbbf24' }}><div className="h-1.5 w-1.5 rounded-full bg-current" />{event.confirmed ? 'Confirmado' : 'Pendente'}</span></div></div>; })}</div>}</div>
    </div>
  );
}
