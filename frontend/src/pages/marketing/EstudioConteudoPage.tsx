...
export default function EstudioConteudoPage() {
  ...
  useEffect(()=>{void load();},[]);
  useEffect(()=>{
    setEdit({caption:selected?.caption||'',scheduled_at:localInput(selected?.scheduled_at)}); setArtUrl('');
  ...
  async function uploadEditedArt(file?:File) {
    ...
    try{const body=new FormData();body.append('file',file);await api.post(`/marketing/content/${selected.id}/art/upload`,body);await load(selected.id);toast.success('Arte editada recebida e enviada para nova aprovação.');}
    ...
    <div className="grid min-h-[620px] overflow-hidden rounded-2xl border bg-white lg:grid-cols-[370px_minmax(0,1fr)]" style={{borderColor:'var(--erp-border)'}}>
      ...
      <section className="min-w-0">{!selected?<div className="flex h-full min-h-[520px] flex-col items-center justify-center text-center"><Sparkles size={30} style={{color:'var(--erp-text-dim)'}}/><p className="mt-4 font-semibold">Selecione uma publicação</p></div>:<>
        ...
        <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[minmax(280px,440px)_1fr]">
          <div><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Arte</p>{canEdit&&<button disabled={!!busy} onClick={()=>run('generate',()=>api.post(`/marketing/content/${selected.id}/generate`),'Nova versão pronta para aprovação.')} className="flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-xs font-semibold text-white disabled:opacity-60" style={{background:'var(--erp-violet)'}}>{busy==='generate'?<Loader2 size={15} className="animate-spin"/>:<Sparkles size={15}/>}Gerar {selected.art_ready?'nova versão':'arte e legenda'}</button>}</div><div className="aspect-[4/5] overflow-hidden rounded-2xl border" style={{borderColor:'var(--erp-border)',background:'var(--erp-surface-2)'}}>{artUrl?<img src={artUrl} alt={selected.title} className="h-full w-full object-cover"/>:<div className="flex h-full flex-col items-center justify-center text-center"><Sparkles size={28}/><p className="mt-3 text-sm font-medium">Arte ainda não gerada</p><p className="mt-1 text-xs" style={{color:'var(--erp-text-muted)'}}>Formato retrato 4:5</p></div>}</div>{selected.art_ready&&canEdit&&<div className="mt-3 border-t pt-3" style={{borderColor:'var(--erp-border)'}}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">Editar no Canva</p><p className="mt-1 text-xs leading-relaxed" style={{color:'var(--erp-text-muted)'}}>Baixe o PPTX 4:5. No Canva, importe o arquivo; título, logo e elementos da marca poderão ser editados.</p></div><a href="https://www.canva.com/" target="_blank" rel="noreferrer" className="flex min-h-9 shrink-0 items-center gap-1.5 text-xs font-semibold" style={{color:'var(--erp-violet)'}}>Abrir Canva<ExternalLink size={13}/></a></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><button disabled={!!busy} onClick={()=>void downloadCanva()} className="flex min-h-10 items-center justify-center gap-2 rounded-xl border text-xs font-semibold disabled:opacity-50" style={{borderColor:'var(--erp-border)'}}>{busy==='canva'?<Loader2 size={14} className="animate-spin"/>:<Download size={14}/>}Baixar editável</button><button disabled={!!busy} onClick={()=>uploadRef.current?.click()} className="flex min-h-10 items-center justify-center gap-2 rounded-xl border text-xs font-semibold disabled:opacity-50" style={{borderColor:'var(--erp-border)'}}>{busy==='upload'?<Loader2 size={14} className="animate-spin"/>:<Upload size={14}/>}Enviar arte editada</button></div><input ref={uploadRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={event=>void uploadEditedArt(event.target.files?.[0])}/><p className="mt-2 text-[11px]" style={{color:'var(--erp-text-dim)'}}>Envie PNG, JPG ou WEBP em 4:5 · recomendado: 1080 × 1350 px · até 15 MB.</p></div>}</div>
