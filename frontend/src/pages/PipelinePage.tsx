...
function KanbanColumn({ stage, deals, activeId }: { stage: (typeof STAGES)[number]; deals: Deal[]; activeId: string | null }) {
  ...
  return (
    <div className="flex w-[82vw] flex-none flex-col rounded-2xl sm:w-64" style={{ background: 'var(--erp-surface)', border: `1px solid ${isOver ? stage.color + '66' : 'var(--erp-border)'}`, minWidth: 240 }}>
  ...
  return (
    <div className="flex flex-col gap-6 h-full">
      ...
      <div className="flex items-end justify-between gap-4">
        <div><p className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: 'var(--erp-violet-light)' }}>Comercial</p><h1 className="text-2xl font-bold" style={{ color: 'var(--erp-text)' }}>Pipeline de Vendas</h1></div>
      </div>
      ...
      {loading ? <div className="py-12 text-sm" style={{ color: 'var(--erp-text-muted)' }}>Carregando pipeline...</div> : deals.length === 0 ? <div className="rounded-2xl py-16 text-center text-sm" style={{ border: '1px dashed var(--erp-border)', color: 'var(--erp-text-muted)' }}>Nenhum lead cadastrado para exibir no pipeline.</div> : <div className="flex-1 overflow-x-auto pb-4"><DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}><div className="flex h-full gap-3 sm:gap-4" style={{ minWidth: 'max-content' }}>{STAGES.map((stage) => <KanbanColumn key={stage.id} stage={stage} deals={deals.filter((d) => d.stage === stage.id)} activeId={activeId} />)}</div><DragOverlay>{activeDeal ? <DealCardOverlay deal={activeDeal} /> : null}</DragOverlay></DndContext></div>}
    </div>
  );
}
