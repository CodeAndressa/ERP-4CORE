...
const DATA: Followup[] = [
  { id: 1, name: 'João Silva', company: 'Construtora Atlas', lastContact: '2026-06-20', nextContact: '2026-06-28', type: 'ligacao', status: 'ativo', priority: 'alta', notes: 'Proposta enviada, aguardando retorno' },
  ...
  { id: 3, name: 'Pedro Oliveira', company: 'Indústria Teksa', lastContact: '2026-06-22', nextContact: '2026-07-02', type: 'reuniao', status: 'ativo', priority: 'alta', notes: 'Segunda reunião agendada' },
  ...
  { id: 5, name: 'Carlos Ferreira', company: 'Tech Soluções', lastContact: '2026-06-24', nextContact: '2026-06-27', type: 'ligacao', status: 'ativo', priority: 'alta', notes: 'Decisão iminente, urgente' },
  { id: 6, name: 'Lucia Mendes', company: 'Grupo Farma', lastContact: '2026-06-18', nextContact: '2026-07-08', type: 'reuniao', status: 'espera', priority: 'media', notes: 'Enviado material de apresentação' },
...
// Alterado de React.ReactNode para JSX.Element
const TYPE_ICONS: Record<ContactType, JSX.Element> = {
  ligacao: <Phone size={13} className="text-violet-400" />,
  email: <Mail size={13} className="text-blue-400" />,
  reuniao: <Users size={13} className="text-emerald-400" />,
...
function DaysBadge({ iso }: { iso: string }) {
  ...
  if (days < 0) {
    return <span className="text-xs font-semibold" style={{ color: '#f87171' }}>{Math.abs(days)}d em atraso</span>;
  }

  if (days === 0) {
    return <span className="text-xs font-semibold" style={{ color: '#fbbf24' }}>Hoje</span>;
  }

  if (days <= 2) {
    return <span className="text-xs font-semibold" style={{ color: '#fbbf24' }}>em {days}d</span>;
  }
              ...
              {filtered.map((f, i) => {
                ...
                return (
                  <tr
                    ...
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLElement).style.background = isUrgent ? 'rgba(248,113,113,0.04)' : 'transparent';
                    }}
                  >
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: pc.bg, color: pc.color }}>
                        {pc.label}
                      </span>
                    </td>
