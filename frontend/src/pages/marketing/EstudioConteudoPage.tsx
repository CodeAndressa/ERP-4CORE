...
import { AlertCircle, Brain, CalendarClock, Camera, Check, ChevronRight, Download, ExternalLink, Lightbulb, Loader2, MessageSquarePlus, Plus, RefreshCw, Send, Smartphone, Sparkles, ThumbsDown, ThumbsUp, Trash2, Upload, X } from 'lucide-react';
import toast from 'react-hot-toast';
import { api } from '../../services/api';
import { kindVisual } from '../../shared/scheduleKinds';
...
type Learning = { total:number; liked:number; disliked:number; copy_rules:string[]; image_rules:string[]; distilled_at:string|null; pending:number; distill_minimum:number };

// Espelha ASPECT_LABELS do backend — cada tag decide se a lição vai para o prompt
// da imagem ou para o da redação, então os valores precisam bater exatamente.
const aspectList = [['imagem','Imagem'],['cores','Cores'],['texto','Frase'],['tom','Tom'],['legibilidade','Legibilidade'],['marca','Logo e link']] as const;
...
export default function EstudioConteudoPage() {
  ...
  const [composer,setComposer]=useState<''|Sentiment|'reject'>('');
  const [notes,setNotes]=useState(''), [aspects,setAspects]=useState<string[]>([]);
  const [feedback,setFeedback]=useState<Feedback[]>([]);
  const [learning,setLearning]=useState<Learning|null>(null), [showLearning,setShowLearning]=useState(false);
  ...
  function openComposer(mode:''|Sentiment|'reject') { setComposer(mode); setNotes(''); setAspects([]); }
  ...
  async function sendFeedback() {
    ...
    try {
      if(composer==='reject'){
        await api.post(`/marketing/content/${selected.id}/reject`,{reason:notes,aspects});
        toast.success('Ajustes registrados. A IA já aprendeu e vai aplicar na próxima versão.');
      } else {
        await api.post(`/marketing/content/${selected.id}/feedback`,{sentiment:composer||'disliked',notes,aspects});
        toast.success(composer==='liked'?'Anotado. A IA vai manter esse padrão.':'Anotado. A IA não vai repetir isso.');
      }
      openComposer(''); await load(selected.id); await loadLearning();
      const {data}=await api.get<{items:Feedback[]}>(`/marketing/content/${selected.id}/feedback`); setFeedback(data.items);
    } catch(e){toast.error(message(e),{duration:6000});} finally{setBusy('');}
  }
  ...
  const canModify=!!selected&&!['generating','publishing','published'].includes(selected.status);
  ...
  const locked=selected?.status==='scheduled'&&!editingScheduled;
  const canEdit=canModify&&!locked;
  ...
  return <div className="mx-auto max-w-[1500px] space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-4 border-b pb-5" style={{borderColor:'var(--erp-border)'}}>
      ...
      <div className="flex flex-wrap gap-2"><button onClick={()=>setShowLearning(v=>!v)} className="flex min-h-11 items-center gap-2 rounded-xl border bg-white px-4 text-sm font-semibold" style={{borderColor:'var(--erp-border)',color:'var(--erp-violet)'}}><Brain size={16}/>O que a IA aprendeu{learning?.total?<span className="rounded-full px-1.5 text-[10px]" style={{background:'var(--erp-violet-dim)'}}>{learning.total}</span>:null}</button><button onClick={()=>suggestions.length?setShowSuggestions(v=>!v):void loadSuggestions()} className="flex min-h-11 items-center gap-2 rounded-xl border bg-white px-4 text-sm font-semibold" style={{borderColor:'var(--erp-border)',color:'var(--erp-violet)'}}><Lightbulb size={16}/>Ideias da IA</button><button onClick={()=>setCreating(true)} className="flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-semibold text-white" style={{background:'var(--erp-violet)'}}><Plus size={16}/>Nova publicação</button></div>
    </div>

    {showLearning&&<section className="overflow-hidden rounded-2xl border bg-white" style={{borderColor:'var(--erp-border)'}}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3 sm:px-5" style={{borderColor:'var(--erp-border)'}}><div><h2 className="flex items-center gap-2 font-semibold"><Brain size={17} style={{color:'var(--erp-violet)'}}/>O que a IA aprendeu com você</h2><p className="mt-1 text-xs" style={{color:'var(--erp-text-muted)'}}>Cada feedback dado numa arte entra no prompt de todos os stories seguintes, automaticamente.</p></div><button aria-label="Fechar aprendizado" onClick={()=>setShowLearning(false)} className="flex h-10 w-10 items-center justify-center"><X size={16}/></button></div>
      {!learning?.total?<div className="p-8 text-center"><Brain className="mx-auto mb-3" size={24} style={{color:'var(--erp-text-dim)'}}/><p className="text-sm font-medium">Nenhum feedback ainda</p><p className="mx-auto mt-1 max-w-md text-xs" style={{color:'var(--erp-text-muted)'}}>Use os botões Gostei e Não gostei embaixo de cada arte de story. A partir do primeiro feedback a próxima geração já sai diferente.</p></div>:<div className="space-y-4 p-4 sm:p-5">
        <div className="flex flex-wrap gap-2 text-xs"><span className="rounded-full px-2.5 py-1 font-semibold" style={{background:'var(--erp-violet-dim)',color:'var(--erp-violet)'}}>{learning.total} feedback{learning.total>1?'s':''}</span><span className="rounded-full bg-emerald-50 px-2.5 py-1 font-semibold text-emerald-700">{learning.liked} aprovado{learning.liked===1?'':'s'}</span><span className="rounded-full bg-amber-50 px-2.5 py-1 font-semibold text-amber-700">{learning.disliked} reprovado{learning.disliked===1?'':'s'}</span>{learning.total<learning.distill_minimum&&<span className="rounded-full px-2.5 py-1" style={{background:'var(--erp-surface-2)',color:'var(--erp-text-muted)'}}>Regras condensadas a partir de {learning.distill_minimum} feedbacks</span>}</div>
        {(learning.copy_rules.length||learning.image_rules.length)?<div className="grid gap-4 md:grid-cols-2">
          <div><p className="mb-2 text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Regras de frase e tom</p><ul className="space-y-1.5">{learning.copy_rules.map((rule,i)=><li key={i} className="rounded-xl px-3 py-2 text-xs leading-relaxed" style={{background:'var(--erp-surface-2)'}}>{rule}</li>)}{!learning.copy_rules.length&&<li className="text-xs" style={{color:'var(--erp-text-dim)'}}>Nada consolidado ainda.</li>}</ul></div>
          <div><p className="mb-2 text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Regras de imagem</p><ul className="space-y-1.5">{learning.image_rules.map((rule,i)=><li key={i} className="rounded-xl px-3 py-2 text-xs leading-relaxed" style={{background:'var(--erp-surface-2)'}}>{rule}</li>)}{!learning.image_rules.length&&<li className="text-xs" style={{color:'var(--erp-text-dim)'}}>Nada consolidado ainda.</li>}</ul></div>
        </div>:<p className="rounded-xl p-3 text-xs" style={{background:'var(--erp-violet-dim)',color:'var(--erp-violet)'}}>Seus {learning.total} feedback{learning.total>1?'s':''} já estão valendo nas gerações, na íntegra. A partir de {learning.distill_minimum} a IA condensa tudo em regras curtas e elas aparecem aqui.</p>}
      </div>}
    </section>}
    ...
    <div className="grid min-h-[620px] overflow-hidden rounded-2xl border bg-white lg:grid-cols-[370px_minmax(0,1fr)]" style={{borderColor:'var(--erp-border)'}}>
      <section className="border-b lg:border-b-0 lg:border-r" style={{borderColor:'var(--erp-border)'}}>
        ...
        <div className="max-h-[560px] overflow-y-auto">{loading&&<div className="space-y-2 p-3">{[1,2,3,4].map(i=><div key={i} className="h-[88px] animate-pulse rounded-xl" style={{background:'var(--erp-surface-2)'}}/>)}</div>}{!loading&&!visible.length&&<div className="p-10 text-center"><Sparkles className="mx-auto mb-3" size={24} style={{color:'var(--erp-text-dim)'}}/><p className="text-sm font-medium">Nada neste filtro</p></div>}{!loading&&visible.map(item=><button key={item.id} onClick={()=>setSelectedId(item.id)} className="flex min-h-[104px] w-full items-center gap-3 border-b px-4 py-3 text-left" style={{borderColor:'var(--erp-border)',background:item.id===selectedId?'var(--erp-violet-dim)':'#fff'}}><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{background:kindVisual(item.layout).soft,color:kindVisual(item.layout).ink}}>{item.layout==='story'?<Smartphone size={18}/>:<Camera size={18}/>}</span><span className="min-w-0 flex-1"><b className="block truncate text-sm">{item.title}</b><span className="mt-1.5 flex items-center gap-1.5"><Pill status={item.status}/><span className="rounded-full px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide" style={{background:kindVisual(item.layout).soft,color:kindVisual(item.layout).ink}}>{kindVisual(item.layout).label}</span></span>{item.scheduled_at&&<small className="mt-1.5 flex items-center gap-1" style={{color:'var(--erp-text-muted)'}}><CalendarClock size={11}/>{new Date(item.scheduled_at).toLocaleString('pt-BR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'})}</small>}</span><ChevronRight size={15}/></button>)}</div>
      </section>
      ...
      <section className="min-w-0">{!selected?<div className="flex h-full min-h-[520px] flex-col items-center justify-center text-center"><Sparkles size={30} style={{color:'var(--erp-text-dim)'}}/><p className="mt-4 font-semibold">Selecione uma publicação</p></div>:<>
        <div className="flex items-start justify-between gap-3 border-b p-4 sm:p-5" style={{borderColor:'var(--erp-border)'}}><div><Pill status={selected.status}/><h2 className="mt-2 text-lg font-bold">{selected.title}</h2><p className="text-xs" style={{color:'var(--erp-text-muted)'}}>{selected.layout==='story'?'Story 9:16':'Retrato 4:5'} · {selected.channel}</p></div>{canModify&&<button onClick={()=>run('delete',()=>api.delete(`/marketing/content/${selected.id}`),'Publicação excluída.').then(()=>load())} className="flex h-10 w-10 items-center justify-center rounded-xl border text-red-700" style={{borderColor:'var(--erp-border)'}}><Trash2 size={15}/></button>}</div>
        {selected.revision_notes&&<div className="m-4 rounded-xl border p-3 sm:m-5" style={{borderColor:'#fedf89',background:'#fffaeb'}}><p className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-amber-800"><MessageSquarePlus size={13}/>Ajustes pedidos nesta peça</p><p className="mt-1.5 whitespace-pre-wrap text-xs leading-relaxed text-amber-900">{selected.revision_notes}</p><p className="mt-2 text-[11px] text-amber-700">A próxima versão gerada precisa resolver todos eles.</p></div>}
        ...
        <div className="grid gap-5 p-4 sm:p-5 xl:grid-cols-[minmax(280px,440px)_1fr]">
          <div><div className="mb-2 flex items-center justify-between gap-3"><p className="text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Arte</p>{canEdit&&selected.layout==='story'&&<button disabled={!!busy} onClick={()=>run('generate',()=>api.post(`/marketing/content/${selected.id}/generate`),'Nova versão pronta para aprovação.')} className="flex min-h-10 items-center justify-center gap-2 rounded-xl px-3 text-xs font-semibold text-white disabled:opacity-60" style={{background:'var(--erp-violet)'}}>{busy==='generate'?<Loader2 size={15} className="animate-spin"/>:<Sparkles size={15}/>}Gerar {selected.art_ready?'nova versão':'story'}</button>}</div><div className={`overflow-hidden rounded-2xl border ${selected.layout==='story'?'aspect-[9/16]':'aspect-[4/5]'}`} style={{borderColor:'var(--erp-border)',background:'var(--erp-surface-2)'}}>{artUrl?<img src={artUrl} alt={selected.title} className="h-full w-full object-cover"/>:<div className="flex h-full flex-col items-center justify-center text-center"><Sparkles size={28}/><p className="mt-3 text-sm font-medium">Arte ainda não gerada</p><p className="mt-1 text-xs" style={{color:'var(--erp-text-muted)'}}>Formato {selected.layout==='story'?'story 9:16':'retrato 4:5'}</p></div>}</div>{selected.layout==='story'&&selected.art_ready&&<div className="mt-3 space-y-3 border-t pt-3" style={{borderColor:'var(--erp-border)'}}>
            {selected.headline&&<div><p className="text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Frase na arte</p><p className="mt-1 text-sm font-semibold leading-snug">{selected.headline}</p></div>}
            <div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">O que você achou desta arte?</p><p className="mt-1 text-xs leading-relaxed" style={{color:'var(--erp-text-muted)'}}>Vale para todos os próximos stories, não só para este. A IA aprende a cada feedback.</p></div></div>
            {!composer&&<div className="grid gap-2 sm:grid-cols-2"><button disabled={!!busy} onClick={()=>openComposer('liked')} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border text-xs font-semibold text-emerald-700 disabled:opacity-50" style={{borderColor:'var(--erp-border)'}}><ThumbsUp size={14}/>Gostei</button><button disabled={!!busy} onClick={()=>openComposer('disliked')} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border text-xs font-semibold text-amber-700 disabled:opacity-50" style={{borderColor:'var(--erp-border)'}}><ThumbsDown size={14}/>Não gostei</button></div>}
            {!!composer&&<div className="space-y-3 rounded-xl border p-3" style={{borderColor:'var(--erp-border)',background:'var(--erp-surface-2)'}}>
              <div className="flex items-center justify-between gap-2"><p className="text-xs font-semibold">{composer==='liked'?'O que funcionou bem?':composer==='reject'?'O que precisa mudar nesta peça?':'O que não funcionou?'}</p><button aria-label="Cancelar" onClick={()=>openComposer('')} className="flex h-8 w-8 items-center justify-center"><X size={14}/></button></div>
              <div><p className="mb-1.5 text-[11px] font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Onde exatamente</p><div className="flex flex-wrap gap-1.5">{aspectList.map(([value,label])=>{const on=aspects.includes(value);return <button key={value} onClick={()=>toggleAspect(value)} className="min-h-9 rounded-full border px-3 text-[11px] font-semibold" style={on?{borderColor:'var(--erp-violet)',background:'var(--erp-violet)',color:'#fff'}:{borderColor:'var(--erp-border)',background:'#fff',color:'var(--erp-text-muted)'}}>{label}</button>;})}</div></div>
              <textarea rows={3} value={notes} onChange={e=>setNotes(e.target.value)} placeholder={composer==='liked'?'Ex.: a foto ficou realista e a frase curta funcionou bem':'Ex.: a pessoa ficou com cara de boneco e o fundo puxou pro azul'} className="w-full resize-none rounded-xl border p-3 text-xs leading-relaxed" style={{borderColor:'var(--erp-border)'}}/>
              <button disabled={!!busy} onClick={()=>void sendFeedback()} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl text-xs font-semibold text-white disabled:opacity-60" style={{background:'var(--erp-violet)'}}>{busy==='feedback'?<Loader2 size={14} className="animate-spin"/>:<Brain size={14}/>}{composer==='reject'?'Registrar ajustes e ensinar a IA':'Ensinar a IA'}</button>
            </div>}
            {!!feedback.length&&<div className="space-y-1.5"><p className="text-[11px] font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Feedback desta arte</p>{feedback.map(item=><div key={item.id} className="flex items-start gap-2 rounded-xl px-3 py-2" style={{background:'var(--erp-surface-2)'}}>{item.sentiment==='liked'?<ThumbsUp size={13} className="mt-0.5 shrink-0 text-emerald-700"/>:<ThumbsDown size={13} className="mt-0.5 shrink-0 text-amber-700"/>}<span className="min-w-0 flex-1"><span className="block text-xs leading-relaxed">{item.notes||'Sem comentário'}</span>{!!item.aspects.length&&<span className="mt-1 block text-[10px] font-semibold uppercase tracking-wide" style={{color:'var(--erp-text-dim)'}}>{item.aspects.join(' · ')}</span>}</span><button aria-label="Remover feedback" disabled={!!busy} onClick={()=>void removeFeedback(item.id)} className="flex h-7 w-7 shrink-0 items-center justify-center disabled:opacity-40">{busy===`fb-${item.id}`?<Loader2 size={12} className="animate-spin"/>:<Trash2 size={12} style={{color:'var(--erp-text-dim)'}}/>}</button></div>)}</div>}
          ...
          <div className="space-y-4">
            {locked&&<div className="space-y-2 rounded-xl border p-4" style={{borderColor:'var(--erp-border)',background:'var(--erp-surface-2)'}}><div className="flex items-center justify-between gap-2"><span className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}><CalendarClock size={13}/>Agendado para</span><button onClick={()=>setEditingScheduled(true)} className="text-xs font-semibold" style={{color:'var(--erp-violet)'}}>Editar agendamento</button></div><p className="text-sm font-semibold">{selected.scheduled_at?new Date(selected.scheduled_at).toLocaleString('pt-BR',{dateStyle:'long',timeStyle:'short'}):'-'}</p>{selected.layout==='feed'&&selected.caption&&<p className="whitespace-pre-wrap text-xs leading-relaxed" style={{color:'var(--erp-text-muted)'}}>{selected.caption}</p>}</div>}
            {!locked&&<>
            {selected.layout==='feed'&&canEdit&&<label className="block space-y-2"><span className="text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Legenda modelo (opcional)</span><textarea rows={3} value={captionRef} onChange={e=>setCaptionRef(e.target.value)} placeholder="Cole uma legenda de outra empresa/post pra IA imitar o estilo — o nome da empresa é sempre trocado por 4Core." className="w-full resize-none rounded-xl border p-3 text-xs leading-relaxed" style={{borderColor:'var(--erp-border)'}}/><button disabled={!!busy} onClick={()=>void suggestCaption()} className="flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border text-xs font-semibold disabled:opacity-50" style={{borderColor:'var(--erp-border)',color:'var(--erp-violet)'}}>{busy==='caption'?<Loader2 size={14} className="animate-spin"/>:<Sparkles size={14}/>}Sugerir legenda{captionRef.trim()?' com base no modelo':' pra esta publicação'}</button></label>}
            {selected.layout==='feed'&&<label className="block space-y-2"><span className="text-xs font-semibold uppercase tracking-wider" style={{color:'var(--erp-text-muted)'}}>Legenda</span><textarea disabled={!canEdit} rows={11} value={edit.caption} onChange={e=>setEdit({...edit,caption:e.target.value})} className="w-full resize-none rounded-xl border p-3 text-sm leading-relaxed disabled:bg-slate-50" style={{borderColor:'var(--erp-border)'}}/></label>}
            {selected.layout==='story'&&<p className="rounded-xl p-3 text-xs" style={{background:'var(--erp-violet-dim)',color:'var(--erp-violet)'}}>Stories não têm legenda na Meta. A frase, o endereço <b>4core.site</b> e a logo já vêm desenhados na própria arte, porque a API da Meta não permite anexar sticker de link em story publicado por aplicativo.</p>}
