...
const EVENTS: AgendaEvent[] = [
  { id: 1, date: '2026-06-26', time: '09:00', title: 'Apresentação de proposta',    contact: 'Pedro Oliveira', company: 'Indústria Teksa',    type: 'reunião',   confirmed: true  },
  { id: 2, date: '2026-06-26', time: '11:00', title: 'Follow-up contrato',          contact: 'Carlos Ferreira', company: 'Tech Soluções',     type: 'ligação',   confirmed: true  },
  ...
  { id: 4, date: '2026-06-27', time: '10:00', title: 'Renovação de contrato',       contact: 'João Silva',      company: 'Construtora Atlas',  type: 'reunião',   confirmed: true  },
  ...
  { id: 6, date: '2026-06-28', time: '09:30', title: 'Prospecção ativa',            contact: 'Lucia Mendes',    company: 'Grupo Farma',        type: 'ligação',   confirmed: false },
  { id: 7, date: '2026-06-29', time: '14:00', title: 'Fechamento de proposta',      contact: 'Roberto Lima',    company: 'Agro Primavera',     type: 'reunião',   confirmed: true  },
  ...
  { id: 10,date: '2026-07-01', time: '16:00', title: 'Apresentação comercial',      contact: 'Carlos Ferreira', company: 'Tech Soluções',     type: 'reunião',   confirmed: false },
];
...
const TYPE_CONFIG: Record<EventType, { icon: React.ReactNode; bg: string; color: string }> = {
  reunião:   { icon: <Users     size={12} />, bg: 'rgba(124,77,255,0.12)',  color: '#7c4dff' },
  ligação:   { icon: <Phone     size={12} />, bg: 'rgba(52,211,153,0.12)', color: '#34d399' },
...
export default function AgendaPage() {
  ...
  const todayEvents = EVENTS.filter((e) => e.date === selectedDay)
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold" style={{ color: 'var(--erp-text)' }}>Agenda Comercial</h1>
          <p className="text-sm mt-1" style={{ color: 'var(--erp-text-muted)' }}>Semana de 26 jun – 2 jul 2026</p>
        </div>
        <button
          ...
          return (
            ...
            >
              ...
              <span className="text-xl font-bold tabular-nums"
                style={{ color: active ? '#fff' : 'var(--erp-text)' }}>
      ...
      <div>
        <p className="text-sm font-semibold mb-3" style={{ color: 'var(--erp-text)' }}>
          {fmtDay(selectedDay)} {selectedDay === '2026-06-26' && (
            <span className="ml-2 rounded-full px-2 py-0.5 text-[10px] font-medium"
              style={{ background: 'var(--erp-violet-dim)', color: 'var(--erp-violet-light)' }}>Hoje</span>
          )}
        </p>
        {todayEvents.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 rounded-2xl py-14"
            style={{ border: '1px dashed var(--erp-border)' }}>
            <Users size={28} style={{ color: 'var(--erp-text-dim)' }} />
            <p className="text-sm" style={{ color: 'var(--erp-text-muted)' }}>Nenhum evento neste dia</p>
          </div>
        ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-shrink-0 w-12 text-center">
                    <span className="text-sm font-bold tabular-nums" style={{ color: 'var(--erp-text)' }}>{event.time}</span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium" style={{ color: 'var(--erp-text)' }}>{event.title}</p>
