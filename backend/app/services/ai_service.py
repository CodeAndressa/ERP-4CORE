...
class AIService:
    ...
    async def analyze(self, scope: str, instructions: str, db: Session) -> dict[str, Any]:
        ...
        system = """Você é a consultora estratégica interna da 4Core. Gere recomendações objetivas em português do Brasil, baseadas apenas nos dados reais fornecidos em "connected_data" para a área "scope" selecionada. Não invente métricas, clientes ou resultados que não estejam no contexto. Quando o contexto vier com "error" ou vazio, declare isso e proponha o próximo dado necessário, em vez de generalizar. Em sugestões orçamentárias, trate valores como estimativas gerenciais, não como aconselhamento financeiro profissional. Responda APENAS JSON válido neste formato: {"headline": string, "summary": string, "actions": [{"area": string, "priority": "alta|media|baixa", "title": string, "rationale": string, "recommended_action": string}], "budget_suggestions": [{"title": string, "suggestion": string}], "content_ideas": [{"title": string, "format": string, "angle": string}]}. Retorne no máximo 5 actions, 3 budget_suggestions e 4 content_ideas."""
