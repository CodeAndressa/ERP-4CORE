...
const TYPE_ICONS: Record<ContactType, JSX.Element> = { ligacao: <Phone size={13} className="text-violet-400" />, email: <Mail size={13} className="text-blue-400" />, reuniao: <Users size={13} className="text-emerald-400" />, whatsapp: <Bell size={13} className="text-green-400" /> };
const PRIORITY_CONFIG: Record<Priority, { bg: string; color: string; label: string }> = { alta: { bg: 'rgba(248,113,113,0.12)', color: '#f87171', label: 'Alta' }, media: { bg: 'rgba(251,191,36,0.12)', color: '#fbbf24', label: 'Média' }, baixa: { bg: 'rgba(100,116,139,0.12)', color: '#94a3b8', label: 'Baixa' } };
...
export default function FollowupPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card padding="sm">{loading ? <div className="py-12 text-center text-sm" style={{ color: 'var(--erp-text-muted)' }}>Carregando follow-up...</div> : filtered.length === 0 ? <div className="py-14 text-center text-sm" style={{ color: 'var(--erp-text-muted)' }}>Nenhum lead para acompanhamento.</div> : <div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr style={{ borderBottom: '1px solid var(--erp-border)' }}>{['Lead', 'Empresa', 'Próximo contato', 'Tipo', 'Prioridade', 'Notas'].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide" style={{ color: 'var(--erp-text-muted)' }}>{h}</th>)}</tr></thead><tbody>{filtered.map((f, i) => { const pc = PRIORITY_CONFIG[f.priority]; const days = daysUntil(f.nextContact); const isUrgent = days <= 1 && f.status === 'ativo'; return <tr key={f.id} style={{ borderBottom: i < filtered.length - 1 ? '1px solid var(--erp-border)' : undefined, background: isUrgent ? 'rgba(248,113,113,0.04)' : undefined }} onMouseEnter={(e) => { if (!isUrgent) e.currentTarget.style.background = 'var(--erp-surface-2)'; }} onMouseLeave={(e) => { e.currentTarget.style.background = isUrgent ? 'rgba(248,113,113,0.04)' : 'transparent'; }}><td className="px-4 py-3"><div className="flex items-center gap-2">{f.status === 'concluido' && <CheckCircle size={12} style={{ color: '#34d399' }} />}{f.status === 'espera' && <Clock size={12} style={{ color: '#fbbf24' }} />}<span className="font-medium" style={{ color: 'var(--erp-text)' }}>{f.name}</span></div></td><td className="px-4 py-3 text-xs" style={{ color: 'var(--erp-text-muted)' }}>{f.company}</td><td className="px-4 py-3"><div className="flex flex-col gap-0.5"><span className="text-xs tabular-nums" style={{ color: 'var(--erp-text-muted)' }}>{f.nextContact ? new Date(f.nextContact + 'T12:00:00').toLocaleDateString('pt-BR') : '-'}</span><DaysBadge iso={f.nextContact} /></div></td><td className="px-4 py-3"><div className="flex items-center gap-1.5">{TYPE_ICONS[f.type]}<span className="text-xs" style={{ color: 'var(--erp-text-muted)' }}>{TYPE_LABELS[f.type]}</span></div></td><td className="px-4 py-3"><span className="rounded-full px-2 py-0.5 text-[10px] font-medium" style={{ background: pc.bg, color: pc.color }}>{pc.label}</span></td><td className="px-4 py-3 text-xs max-w-[260px] truncate" style={{ color: 'var(--erp-text-muted)' }}>{f.notes}</td></tr>; })}</tbody></table></div>}</Card>
    </div>
  );
}
