...
export default function AgendaPage() {
  ...
  useEffect(() => {
    api.get('/leads')
      .then(({ data }) => {
        ...
        setEvents(items.filter((lead) => lead.next_contact_date).map((lead) => ({
          id: lead.id,
  ...
  const weekWithoutAction = weekEvents.filter((e) => e.title === 'Contato comercial').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3"><MetricCard label="Dia selecionado" value={String(todayEvents.length)} detail="contatos previstos" tone="violet" icon={<Users size={16} />} /><MetricCard label="Proximos 7 dias" value={String(weekEvents.length)} detail="contatos previstos" tone="emerald" icon={<Phone size={16} />} /><MetricCard label="Sem ação definida" value={String(weekWithoutAction)} detail="nos próximos 7 dias" tone="amber" icon={<CalendarDays size={16} />} /></div>
